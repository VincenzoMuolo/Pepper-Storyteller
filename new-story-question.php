            ...
            if(!$conn->exec($myCreateStatement)) {
    			...
    			foreach ($_POST['paragraph'] as $textArea) { 
                    ...
                    if(isset($_POST["question"][$counter]) && strlen($_POST["question"][$counter]) > 0){
                     ...
                     //Inizio modifiche per inserimento conclusioni
                     if(isset($_POST["alternateEnding"][$counter])) {
                     		$ending = $_POST["alternateEnding"][$counter];
                            $value = "FT".$ending;
                            if($ending !== null && strlen($ending) > 0) {
                                  $my_AlternateEnding = $conn->prepare("INSERT INTO `Conclusioni` (Storia, IdParagrafo, Termina) VALUES (:title, :id_paragraph, :ending)");
                                  $my_AlternateEnding->bindParam(':title', $title);
                                  $my_AlternateEnding->bindParam(':id_paragraph', $ending);
                                  $my_AlternateEnding->bindParam(':ending', $value);
                                  $my_AlternateEnding->execute();
                            }
                     }
                    //Fine modifiche per inserimento conclusioni
                    //FINE MODIFICA
        				$counter++;
					}
            	...
            	if ($my_Insert_Statement->execute()) {
                    ...
                    header("location: new-test-confirm.html");
...
<html lang="it">
    <head>
        ...
        <script src="create-paragraph-question.js"></script>
        ...
        <div class="topPage">
            <nav id="nav" class="navigation" style="background-color:transparent;">
                ...
                <div class="right-nav-side">
                    ...
                    <a href="new-story-question.php" class="nav-active">NUOVO TEST</a>
            ...
            <div class="main-page">
                <form id="formData" enctype="multipart/form-data" action="<?php echo htmlspecialchars($_SERVER["PHP_SELF"]);?>" method="post">  
                    <div id="top-area" class="top-banner">
                        ...
                        <h2>Crea un test con domande</h2>
                        <h3>In questa sezione è possibile creare test con immagini, paragrafi e domande, impostando anche la possibità di direzionare le risposte verso uno specifico paragrafo</h3>
                        </div>
                        <div id="left-side-banner" class="top-banner-left-side adjust-position">
                    ...
                    <div id="help-panel" class="top-row">
                		<div id="par-column" class="top-row-column">
                	    	...
                	    	<div id="btn-container1" class="button-container">
                  		   		<input id="paragraph-button1" type="button" title="Aggiungi domanda" onclick="createParagraph()" value="AGGIUNGI">
                  		    	<input id="paragraph-button2" type="button" title="Rimuovi domanda" onclick="deleteParagraph()" value="RIMUOVI">
                	    	</div>
                        </div>
                        <div id="rec-column" class="top-row-column-rec">
                        	<input id="upload-story" type="button" value="INVIA TEST">
                        </div>     
                	</div>
